...
/*
double** IFFT::getifftw_channel(int N, double **fftdata, int len) {
	 double [2]

	int count_ifftdata = 0;

	for (int i = 0; i < 12; i++) {
		c_out1[i][0] = fftdata[0][i];
		c_out1[i][1] = fftdata[1][i];
		c_out2[i][0] = fftdata[0][i + 12];
		c_out2[i][1] = fftdata[1][i + 12];

		cout << "" << c_out1[i][0] << endl << c_out1[i][1] << endl;
	}
	cout << "******************" << endl;
	fftw_execute(cq1);
	fftw_execute(cq2);
	 normalize
	for (int i = 0; i < N; i++) {
		cout << c_in1[i][0] << endl << c_in1[i][1] << endl;
		c_in2[i][0] *= 1. / N;
		c_in2[i][1] *= 1. / N;
		c_in1[i][0] *= 1. / N;
		c_in1[i][1] *= 1. / N;
		...
		if ((len * 3 / 2) > count_ifftdata) {
			...
			ifftdata[0][count_ifftdata + 12] = c_in2[i][0];
			ifftdata[1][count_ifftdata + 12] = c_in2[i][1];
			count_ifftdata++;
		}
