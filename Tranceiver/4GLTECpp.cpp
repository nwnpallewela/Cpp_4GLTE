...
int main() {
	for (int var = 0; var < 3; ++var) {
		...
		cout << endl << endl
				<< "*************************************************" << endl
				<< "*************************************************" << endl
				<< "This is a test of LTEA C++ Transmitter implementation "
				<< endl << "*************************************************"
				<< endl << "*************************************************"
				<< endl << endl;
		...
		Channel ch = Channel("High", "EPA 5Hz", 5, 1);
		FFT_12 fft;
		IFFT_CH ifft;
		cx_mat H;
		cx_mat noise;
		...
		for (int i_size = 0; i_size < datafilesize; i_size = i_size + 320) {
			...
			//cout << endl << "Encoded data length : " << encodeddata.length() /6<< endl;
			...
			endtime = clock();
			/*for (int var = 0; var < antenna2_count; ++var) {
			 cout<<antenna1[0][var]<<" + "<<antenna1[1][var]<<"i                           "<<antenna2[0][var]<<" + "<<antenna2[1][var]<<"i"<<endl;
...
//	}
			...
			for (int i = 0; i < 11; i++) {
				...
				/*	cout << "These are antenna outs" << endl;
				 for (int var = 0; var < 12; ++var) {
				 cout << antenna1_ch[0][var] << " + " << antenna1_ch[1][var]
				 << "i                           "
				 << antenna2_ch[0][var] << " + "
				 << antenna2_ch[1][var] << "i" << endl;

				 }
				 cout << endl;*/
				for (int var = 0; var < 16; ++var) {
					S.at(var) = modulateddata[0][i * 16 + var];
					S.at(var + 16) = modulateddata[1][i * 16 + var];
				}

				/*LTEMIMOCHANNEL(antenna1_ch, 12, antenna2_ch, "High", "EPA 5Hz",
				 5, 1);*/
				...
				// Tx_test = fft.getfftw(12, antenna1_ch, 12);
				...
				//cout<<"hhhhhhhhhhhhhhhhhhhh0"<<endl;
				...
				Eq.genH(ch.getHout());
				//Eq.RS(S);
				//	(Y-Eq.getQtRS(S)).print("This is Y- Q-1*RS: ");
				y_ = Eq.GetLSD_Y(Y);

				//	y_ = Eq.GetLSD_Y(Eq.getRS(S));

				/*for (int var = 0; var < 16; ++var) {
				 cout<<y_[var]<<" + "<<y_[var+16]<<" i"<<endl;
				 }*/
			...
			//	cout << endl << received_data << endl;

			int* decode;
			decode = new int[40];
			int* decoded_data;
			decoded_data = new int[rawdata.length()];
			int count = 0;
			//	startTime = System.nanoTime();
			for (int i1 = 0; i1 < encodeddata.length(); i1 = i1 + 132) {

				/*decode = dec.decoder_log_map(received_data.substr(i1, i1 + 131),
				 decoder_num, 40);*/

				dec.decoder_log_map_it(received_data.substr(i1, i1 + 131), 1,
						data_block_size, luk);
				//	LLR1=dec.getLLR1();
				//	A1=dec.get_A();
				...
				R1 = dec.getR();
				y1 = dec.getY();
				dec.decoder_log_map_it(received_data.substr(i1, i1 + 131), 2,
		...
		cout << endl << "total time               = "
				<< (encode_time_full + modulate_time_full + fft_time_full
						+ ifft_time_full + multiplex_time_full) / 1000 << " ms"
				<< endl;

		cout << "total_time_without_dataread = "
				<< total_time_without_dataread / 1000 << " ms" << endl;

		cout << "Error count           = " << error << " / " << rawdata.length()
				<< endl;
